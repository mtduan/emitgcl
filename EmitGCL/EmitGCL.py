...
class EmitGCL(nn.Module):
    """
    EmitGCL is a graph-based model for gene-cell learning, combining Gene Regulatory Network (GNN) 
    with pathway prediction and contrastive learning losses.

    Parameters:
        gnn (nn.Module): 
            A graph neural network (GNN) model for processing gene and cell features.
        
        labsm (float): 
            The label smoothing rate used in the clustering loss function.
            Default is 0.3.
        
        n_hid (int): 
            The number of hidden units in the model. Default is 104.
        
        n_batch (int): 
            The number of batches per training epoch. This parameter controls 
            the batch size for training.
        
        device (torch.device): 
            The device (CPU or GPU) on which the model will run (e.g., 'cuda' or 'cpu').
        
        lr (float): 
            The learning rate for the GNN optimizer. Default is 0.0005.
        
        wd (float): 
            The weight decay parameter for regularization. Default is 0.1.
        
        pathway_genes (dict): 
            A dictionary where keys are pathway names and values are lists of genes associated with those pathways.
        
        pathway_directions (dict): 
            A dictionary where keys are pathway names and values specify the direction of regulation 
            (e.g., 'forward' or 'reverse').
        
        gene_names (list): 
            A list of gene names corresponding to the columns of the input RNA matrix.
        
        sample_type (list): 
            A list that specifies the type of sample for each cell.
        
        sample_list (list): 
            A list of sample pairs for contrastive learning. Each sample pair will be used to calculate the contrastive loss.
        
        num_epochs (int, optional): 
            The number of epochs for training. Default is 1.
    ...
    def __init__(self, gnn, labsm, n_hid, n_batch, device, lr, wd, pathway_genes, pathway_directions, gene_names, sample_type, sample_list, num_epochs=1):
        """
        Initializes the EmitGCL model with the given parameters.

        Arguments:
            gnn (nn.Module): 
                A graph neural network model for processing gene and cell data.
            
            labsm (float): 
                The label smoothing rate.
            
            n_hid (int): 
                The number of hidden units in the model.
            
            n_batch (int): 
                The number of batches per epoch during training.
            
            device (torch.device): 
                The device on which the model will run (e.g., 'cpu' or 'cuda').
            ...
            pathway_genes (dict): 
                A dictionary of pathways and their associated genes.
            
            pathway_directions (dict): 
                A dictionary of pathways and their directions of regulation.
            
            gene_names (list): 
                A list of gene names corresponding to the columns of the RNA matrix.
            
            sample_type (list): 
                A list of sample types for each cell in the RNA matrix.
            
            sample_list (list): 
                A list of sample pairs for contrastive learning.
